...
def format_results_data(data):
    data = data.rename(
        columns={
            "data.init_args.featurizer.init_args.representation": "representation",
            "surrogate_model.init_args.covar_module.init_args.base_kernel.class_path": "kernel",
            "data.init_args.initializer.init_args.method": "init",
            ...
            "acquisition.class_path": "acquisition",
        }
    )
    data["acquisition"].replace(
        {
            "botorch.acquisition.UpperConfidenceBound": "UCB",
            "botorch.acquisition.ExpectedImprovement": "EI",
        },
        inplace=True,
    )
    ...
    data["kernel"].replace(
        {
            "gpytorch.kernels.LinearKernel": "Linear",
            "gpytorch.kernels.MaternKernel": "Matern",
            "chaos.gprotorch.kernels.fingerprint_kernels.tanimoto_kernel.TanimotoKernel": "Tanimoto",
            "gpytorch.kernels.RQKernel": "RQ",
        },
        inplace=True,
    )
    return data
...
from bochemian.gprotorch.metrics import negative_log_predictive_density
...
def visualize_embeddings(embeddings, method="tsne", labels=None):
    """
    ...
    - method: str, optional, default: 'tsne'
      The dimensionality reduction method to use. Either 'tsne' or 'umap'.
    - labels: array-like, shape (n_samples,), optional
      The labels for each point in the embeddings.

    Returns:
    - The transformed data (2D) and a matplotlib figure object.
    """
    if method == "tsne":
        model = TSNE(n_components=2, random_state=42)
    elif method == "umap":
        model = UMAP(n_neighbors=5, min_dist=0.3, random_state=42)
    else:
        raise ValueError("Invalid method. Choose either 'tsne' or 'umap'.")

    transformed_data = model.fit_transform(embeddings)

    fig, ax = plt.subplots()
    scatter = ax.scatter(
        transformed_data[:, 0], transformed_data[:, 1], c=labels, cmap="viridis", s=5
    )

    # Add a color bar if labels are provided
