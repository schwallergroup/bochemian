...
class ContrastiveLossTorch:
    ...
    def build_loss_matrix(self, embs: Tensor, ys: Tensor):
        ...
        if self.hard:
            # dy - dz
            loss[high_dy_filter] = (
                y_distance_matrix[high_dy_filter] - emb_distance_matrix[high_dy_filter]
            )
            # dz
...
class TripletLossTorch:
    ...
    def build_loss_matrix(self, embs: Tensor, ys: Tensor):
        ...
        positive_embs = emb_distance_matrix.where(
            y_distance_matrix <= self.threshold, torch.tensor(0.0).to(embs)
        )
            ...
            if self.soft:
                ...
                if self.eta is not None:
                    ...
                    triplet_losses_for_anchor_i = (
                        triplet_losses_for_anchor_i
                        * self.smooth_indicator(self.threshold - pairs_y[:, 0]).div(
                            self.smooth_indicator(self.threshold)
                        )
                        * self.smooth_indicator(pairs_y[:, 1] - self.threshold).div(
                            self.smooth_indicator(1 - self.threshold)
                        )
                    )
            else:
    ...
    def build_loss_matrix(self, embs: Tensor, ys: Tensor):
        ...
        for ind_a in range(embs.size(0)):
            ...
            assert diff_log_y_dist.shape == diff_log_dist.shape == (m, m), (
                diff_log_y_dist.shape,
                diff_log_dist.shape,
                m,
            )
            valid_aij = (
                diff_log_y_dist < 0
            )  # keep triplet having D(y_a, y_i) < D(y_q, y_j)
